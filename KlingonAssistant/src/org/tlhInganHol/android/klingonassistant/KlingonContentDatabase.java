...
/**
 * Contains logic to return specific entries from the database, and
 * load the database table when it needs to be created.
 */
public class KlingonContentDatabase {
    ...
    // entry {boQwI':n}.
    private static final int DATABASE_VERSION = 201302021;
    ...
    /**
     * Convert a string written in "xifan hol" shorthand to {tlhIngan Hol}.
     * This is a mapping which makes it easier to type, since shifting is unnecessary.
     *
     ...
     * z -> '
     *
     * When replacing "h" with "H", the following must be preserved:
     * ch -> ch
     * gh -> gh
     * tlh -> tlh
     * ngh -> ngh (n + gh)
     * ngH -> ngH (ng + H)
     *
     * TODO: Consider allowing "invisible h".  But this probably makes things too "loose".
     * // c -> ch (but ch -/> chh)
    ...
    public Cursor getEntryMatches(String query) {
        ...
        if (queryEntry.basePartOfSpeechIsUnknown() && queryEntry.getEntryName().length() > 4) {
            ...
            Cursor resultsWithGivenPrefixCursor = getEntriesContainingQuery(looseQuery, /* isPrefix */ true);
            copyCursorEntries(resultsCursor, resultsSet, resultsWithGivenPrefixCursor, /* filter */ true, queryEntry);
        ...
        if (queryEntry.basePartOfSpeechIsUnknown()) {
            ...
            if (queryEntry.getEntryName().length() > 2) {
                Cursor resultsWithGivenQueryCursor = getEntriesContainingQuery(looseQuery, /* isPrefix */ false);
                copyCursorEntries(resultsCursor, resultsSet, resultsWithGivenQueryCursor, /* filter */ false, null);
            ...
            Cursor resultsMatchingDefinitionFromBeginning =
                getEntriesMatchingDefinition(queryBase, /* isPrefix */ true, /* useSearchTags */ false);
            copyCursorEntries(resultsCursor, resultsSet, resultsMatchingDefinitionFromBeginning, /* filter */ false, null);
            ...
            if (queryEntry.getEntryName().length() > 2) {
                Cursor resultsMatchingDefinition =
                    getEntriesMatchingDefinition(queryBase,  /* isPrefix */ false,/* useSearchTags */ false);
                copyCursorEntries(resultsCursor, resultsSet, resultsMatchingDefinition, /* filter */ false, null);
                if (resultsMatchingDefinition != null) {
                    resultsMatchingDefinition.close();
                }

                // Match search tags, from beginning, then anywhere else.
                Cursor resultsMatchingSearchTagsFromBeginning =
                    getEntriesMatchingDefinition(queryBase,  /* isPrefix */ true,/* useSearchTags */ true);
                ...
                Cursor resultsMatchingSearchTags =
                    getEntriesMatchingDefinition(queryBase,  /* isPrefix */ false,/* useSearchTags */ true);
                copyCursorEntries(resultsCursor, resultsSet, resultsMatchingSearchTags, /* filter */ false, null);
    ...
    private void copyCursorEntries(MatrixCursor destCursor, HashSet<Integer> destSet, Cursor srcCursor, boolean filter,
    ...
    private Cursor getEntriesContainingQuery(String queryBase, boolean isPrefix) {
        ...
        try {
            cursor = db.query(true, FTS_VIRTUAL_TABLE, ALL_KEYS,
                ...
                precedingWildcard + queryBase.trim() + "%\"",
    ...
    private Cursor getEntriesMatchingDefinition(String piece, boolean isPrefix, boolean useSearchTags) {
        ...
        Cursor cursor = null;
        try {
            cursor = db.query(true, FTS_VIRTUAL_TABLE, ALL_KEYS, key +
                " LIKE \"" + precedingWildcard + piece.trim() + "%\"",
                null, null, null, null, null);
        } catch(SQLiteException e) {
            // Do nothing.
        }
    ...
    private void parseQueryAsComplexWordOrSentence(String query, MatrixCursor resultsCursor, HashSet<Integer> resultsSet) {
    ...
    public Cursor getEntryById(String entryId, String[] columns) {
        ...
        Cursor cursor = mDatabaseOpenHelper.getReadableDatabase().query(true, FTS_VIRTUAL_TABLE, columns,
            KlingonContentDatabase.KEY_ID + "=" + entryId + "", null, null, null, null, null);
        if( cursor != null ) {
            cursor.moveToFirst();
        }
    ...
    private static class KlingonDatabaseOpenHelper extends SQLiteOpenHelper {
        ...
        private String dottedVersion(int version) {
            ...
            return s.substring(0,4) + "." + s.substring(4,6) + "." +
                s.substring(6,8) + Character.toString((char) (s.charAt(8) - '0' + 'a'));
        }
        ...
        /**
         * Initialises the database by creating an empty database and writing to
         * it from application resource.
         */
        ...
        /**
         * Check if the database already exists so that it isn't copied every time the activity is started.
        ...
        private boolean checkDBExists() {
            ...
            try {
                ...
                // The database doesn't exist yet.  It's fine to do nothing
                // here, we just want to return false at the end.
                // Log.d(TAG, "SQLiteCantOpenDatabaseException thrown: " + e);

            } catch(SQLiteDatabaseLockedException e) {
                // The database is locked.  Also return false.
            ...
            // Log.d(TAG, "checkDB == null: " + (checkDB == null));
            return ( checkDB != null );
            */
        }

        /**
         * Copies the database from the application resources' assets folder to the newly
         * created database in the system folder.
         */
